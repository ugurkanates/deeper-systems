...
from environment.env import TicTacToeEnv, agent_by_mark,\
    next_mark, check_game_status, after_action_state
from environment.utils import set_log_level_by
from human_agent import HumanAgent
from agent import Agent

# DEFAULT VALUES FOR TRAINING TD-LEARNING AGENT
...
class TDAgent(object):
    def __init__(self, mark, epsilon, alpha,env):
        self.mark = mark
        self.alpha = alpha
        self.epsilon = epsilon
        self.episode_rate = 1.0
        self.env = env # env we operate on. delegation ftw.
    ...
    def ask_value(self, state):
        ...
        if state not in st_values:
            ...
            val = DEFAULT_VALUE
            # win
            if gstatus > 0: # CALCULATE REWARD 
                val = self.env._calculate_reward(gstatus)
...
def _learn(max_episode, epsilon, alpha, save_file):
    ...
    env = TicTacToeEnv()
    agents = [TDAgent('O', epsilon, alpha,env),
              TDAgent('X', epsilon, alpha,env)]
...
def _play(load_file, vs_agent, show_number):
    ...
    td_agent = TDAgent('X', 0, 0,env)  # prevent exploring
    start_mark = 'O'
